...
import { getFailsafeGroups } from '@/models/failsafeGroups';
...
export function FailsafesPage() {
  const vehicleType = useVehicleStore((s) => s.type);
  const groups = useMemo(() => getFailsafeGroups(vehicleType), [vehicleType]);

  return (
    <div className="w-full space-y-4">
      <div>
        <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Failsafes</h1>
        <p className="mt-1 text-lg text-muted">
          Configure automated responses to signal loss, low battery, and other failures.
        </p>
      </div>
      ...
      <div className="flex items-start gap-3 rounded border border-warning/30 bg-warning/5 px-4 py-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-muted">
            Failsafe actions determine what your vehicle does in an emergency. Test these on the
            ground before flying. Incorrect settings can lead to flyaways or crashes.
          </p>
        </div>
      </div>

      {groups.map((group) => (
        <FailsafeGroup key={group.id} group={group} />
      ))}
    </div>
  );
}
...
function FailsafeGroup({ group }: { group: ParamGroup }) {
  const [expanded, setExpanded] = useState(true);
  const Icon = ICONS[group.icon] || ShieldAlert;
      ...
      {expanded && (
        <div className="border-t border-border">
          {group.fields.map((field) => (
            <FailsafeField key={field.param} field={field} />
          ))}
        </div>
      )}
    </div>
...
function FailsafeField({ field }: { field: ParamField }) {
